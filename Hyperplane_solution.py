...
def image_from_input(Input):
    Input = torch.Tensor(Input).to(device)
    ImageRaw = G(Input)
    Image = ImageRaw.reshape(1, 28, 28)
...
def initialization():
    ...
    # Initialize tensors to start
    InputA = torch.randn(latent_size).to(device)
    InputB = torch.randn(latent_size).to(device)
    ...
    # First convert the input to an output
    ImageARaw = G(InputA)

    # Then reshape it from a 748 tensor to a 1 by 28 by 28 tensor
    ImageA = ImageARaw.reshape(1, 28, 28)

    # Then pull it from the gpu and turn it into a numpy array
    ImageA = ImageA.cpu().detach().numpy()

    # Lastly, show the whole image in grayscale
    plt.imshow(ImageA[0, 0:28, 0:28], cmap='gray');
    plt.show()

    # Same thing for the second input
    ImageBRaw = G(InputB)
    ImageB = ImageBRaw.reshape(1, 28, 28)
    ImageB = ImageB.cpu().detach().numpy()
    plt.imshow(ImageB[0, 0:28, 0:28], cmap='gray');
    plt.show()

    # Once the user has seen the two images they indicate which one they prefer by inputting either "a" or "b"
...
# make and show images from inputs >>> user makes choice >>> update bounds based on choice >>>
...
def iterative_cycle(upper_bound_list, lower_bound_list, InputA, InputB):
    print(InputA)
    print(InputB)
    image_from_input(InputA)
    image_from_input(InputB)
    choice = input("enter choice(a/b/d/p):")
    if choice == "d":
        print("All done!")
        return
    if choice == "p":
    ...
    iterative_cycle(upper_bound_list, lower_bound_list, InputC, InputD)
